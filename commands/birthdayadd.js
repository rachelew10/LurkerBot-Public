...
module.exports.run = async (bot, message, args, con) => {
    //Admin check
  if(!message.member.hasPermission("ADMINISTRATOR")) return message.reply("You must be an admin to add birthdays");
    ...
    con.query(`SELECT bday FROM alltime WHERE user = '${mention.id}' AND guild = '${message.guild.id}'`, function (error, results, fields) {
        ...
        if (args[1].length === 10) {
            ...
            message.channel.send(`Birthday set to ${date} for ${bot.users.get(mention.id).username}`);
        };
...
module.exports.help = {
    name: "birthdayadd",
    usage: "``prefix`` @member DD/MM/YYYY",
