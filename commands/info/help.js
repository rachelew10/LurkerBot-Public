...
module.exports.run = async (bot, message, args) => {
    ...
    if (args[0]) {
        ...
        if (!args[0]) {
            ...
            //.addField('\u200B', '\u200B')
            .addField(`__**Utility Commands:**__`, "``stats`` | ``statsall`` | ``top10`` | ``top10all`` | ``inactive`` | ``inactiveall`` | ``mybirthday`` |  ``profile`` | ``report`` | ``avatar`` | ``timer`` | ``number``")
            //.addField('\u200B', '\u200B')
            .addField(`__**Fun Commands:**__`, "``shame`` | ``gif`` | ``urban`` | ``coinflip``")
            //.addField('\u200B', '\u200B')
